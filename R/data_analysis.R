...
#'
#' @param dataset An [lcms_dataset_family] like object
#' @inheritParams random_subsampling
#' @inheritParams lcms_data_analysis
#'
...
#' subsets.
#' @noRd
split_double_cv <- function(dataset, keep_together = NULL, balance_in_train = NULL,
                            external_val = list(iterations = 16L, test_size = 0.25),
                            internal_val = list(iterations = 10L, test_size = 0.25)) {
    if (is.null(keep_together)) {
        keep_together_data <- seq_len(dim(dataset)[1])
    } else {
        #keep_together_data <- as.factor(lcms_meta_get_column(dataset, keep_together))
        keep_together_data <- as.factor(keep_together)
    }
    sample_idx <- seq_len(dim(dataset)[1])
    ...
    inner_cv_iterations <- purrr::flatten(
        purrr::imap(outer_cv_loop, function(outer, outer_idx) {
            ...
            inner_cv <- random_subsampling(sample_idx = calib_idx,
                                           ...
                                           balance_in_train = balance_in_train[calib_idx])
...
#' @inheritParams lcms_data_analysis
#' @inheritParams lcms_data_analysis_method
#'
...
#'
#' @inheritParams lcms_data_analysis
#' @inheritParams new_lcms_data_analysis_method
...
#' Data analysis
#'
#' Data analysis on AlpsLCMS can be performed on both [lcms_dataset_1D] full spectra
#' as well as [lcms_dataset_peak_table] peak tables.
#'
...
#' [new_lcms_data_analysis_method()]), but for now only a PLSDA classification
...
#' @param data_analysis_method An [lcms_data_analysis_method] object
...
#' @export
lcms_data_analysis <- function(dataset,
                              y_column,
                              identity_column,
                              balance_in_train = NULL,
                              external_val,
...
#' @param dataset An lcms_dataset object
#' @param train_index set of index used to generate the bootstrap datasets
#' @param y_column A list with the clases of the samples
...
#'
#' Bootstrap and permutation over PLS-VIP on Alpslcms can be performed on both
#' [lcms_dataset_1D] full spectra as well as [lcms_dataset_peak_table] peak tables.
#'
#' Use of the bootstrap and permutation methods for a more robust
#' variable importance in the projection metric for partial least
...
#' @param ncomp number of components for the bootstrap models, if NULL it will
...
bp_kfold_VIP_analysis <- function(dataset,
    ...
    #       ncomp = 7
    #     )
    #   #TODO seleccionar varianza mayor que 0.05
    #   # incluir todo esto dentro del propio
    #   #TODO comprobar cual de las dos hay que coger
    #   ncomp = sum(model$explained_variance$X > 0.05)
...
#' Create method for lcms data analysis
#'
...
#'    The return value is returned by `lcms_data_analysis`
#'
#' @return An object encapsulating the method dependent functions that can be used with [lcms_data_analysis]
#' @name lcms_data_analysis_method
#' @export
new_lcms_data_analysis_method <- function(train_evaluate_model,
                                         train_evaluate_model_params_inner,
                                         choose_best_inner,
                                         train_evaluate_model_params_outer,
    ...
    class(out) <- "lcms_data_analysis_method"
    out
}
                                   ...
                                   nPerm = 50)
{
    ...
    for (p in seq_len(nPerm)) {
        ...
        permMod=lcms_data_analysis(dataset_perm,
                                  y_column = YPerm,
                                  identity_column = identity_column,
                                  balance_in_train = balance_in_train,
                                  external_val = external_val,
...
#'
#' @param lcms_data_analysis_model A lcms_data_analysis_model
...
#' @export
permutation_test_plot = function (lcms_data_analysis_model,
                                  permMatrix,
                                  xlab = "AUCs",
                                  xlim,
                                  ylim = NULL,
                                  ...
                                  main = "Permutation test")
{
    ...
    model_auc = mean(lcms_data_analysis_model$outer_cv_results_digested$auroc$auc)
...
#'
#' @param model A lcms_data_analysis_model
#'
